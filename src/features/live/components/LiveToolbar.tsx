...
import { SearchRounded, EditNoteRounded, TuneRounded, CheckRounded } from '@mui/icons-material';
...
import { MOCK_LIVE_PUBLICATIONS } from '../mockData';
...
interface Props {
  ...
  sortBy: SortOption;
  searchQuery: string;
  onCategoryChange: (cat: LiveCategory) => void;
...
export const LiveToolbar: React.FC<Props> = React.memo(
  ({ activeCategory, sortBy, searchQuery, onCategoryChange, onSortChange, onSearchChange }) => {
    ...
    const categoryCounts = useMemo(() => {
      const counts = new Map<LiveCategory, number>();
      counts.set('all', MOCK_LIVE_PUBLICATIONS.length);
      for (const item of MOCK_LIVE_PUBLICATIONS) {
      ...
      <div className={styles.toolbar}>
        ...
        {activeCategory !== 'all' && (
          <span className={styles.activeCategoryPill}>
            {LIVE_CATEGORY_LABELS[activeCategory]}
            <button
              type="button"
        ...
        <Link to={ROUTES.POST_CONSTRUCTOR} className={styles.createButton}>
          <EditNoteRounded sx={{ fontSize: 15 }} />
          Публикация
        </Link>
      </div>
    );
